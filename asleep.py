...
def masscan(filescan, threads, resume):
    ...
    if resume:
        ...
        params = " -p %s -iL %s -oL %s --rate=%s %s" % (
            ",".join(config.global_ports), filescan, config.tmp_masscan_file, threads,
            config.additional_masscan_params())
    ...
    if not Path(config.tmp_masscan_file).exists():
        config.logging.error(
            "Masscan output error, results file %s not found. Try to run Masscan as Administrator (root)" %
...
def get_args():
    ...
    if args.random_country:
        ...
        while config.max_ips < count:
            ...
            config.logging.info(
                "Generated %s IPs from %s" % (total_count, list(dict.fromkeys(config.random_countries))))
